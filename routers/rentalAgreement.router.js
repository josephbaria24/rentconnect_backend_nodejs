...
// POST: Create a rental agreement draft
router.post('/rental-agreement/rent', async (req, res) => {
    const { occupantId, landlordId, roomId, leaseEndDate, terms } = req.body;
  
    try {
      // Fetch the inquiry that corresponds to the roomId
      const inquiry = await Inquiry.findOne({ roomId, status: 'approved', requestType: 'rent' });
  
      if (!inquiry) {
        return res.status(404).json({ error: 'No approved rent found for this room' });
      }
...
router.get('/rental-agreement/inquiry/:inquiryId', async (req, res) => {
  try {
      ...
      // Check if inquiry exists and is approved
      if (!inquiry || inquiry.status !== 'approved') {
          return res.status(404).json({ error: 'Approved inquiry not found' });
      }

      // Check if the inquiry is for 'reservation' or 'rent'
      if (inquiry.requestType !== 'reservation' && inquiry.requestType !== 'rent') {
          return res.status(400).json({ error: `Inquiry type '${inquiry.requestType}' is not supported` });
      }
